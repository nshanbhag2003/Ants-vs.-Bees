...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        steps = 0
        position = self.place
        while position.entrance is not None: 
            if steps > self.max_range: 
                return None
            if not position.is_hive: 
                if len(position.bees) > 0 and steps >= self.min_range: 
    ...
    def action(self, gamestate):
        ...
        while position:
            if position.ant is not None:
                if not position.ant.is_doubled:
                    position.ant.is_doubled = True
                    position.ant.double()
                if position.ant.is_container and position.ant.ant_contained is not None:
                    if not position.ant.ant_contained.is_doubled:
                        position.ant.ant_contained.double()
                        position.ant.ant_contained.is_doubled = True
...
class Bee(Insect):
    ...
    def slow(self, num_turns):
        """Slow the bee for a further NUM_TURNS turns."""
        # BEGIN Problem EC
        self.is_slow = True
        self.slow_turns += num_turns
        # END Problem EC

    def scare(self, num_turns):
        """
        If this Bee has not been scared before, cause it to attempt to
        go backwards NUM_TURNS times.
        """
        # BEGIN Problem EC
